...
def test_reference_cache_hit_returns_independent_tensors(tmp_path):
    ...
    vae, encoder = _encoder()
    original = encoder._encode(str(path), "right")
    expected = original.clone()
    original.zero_()
    cached = encoder._encode(str(path), "right")
    torch.testing.assert_close(cached, expected, rtol=0, atol=0)
    assert len(vae.inputs) == 1
    assert encoder._service.stats()["hits"] == 1
...
def test_same_path_same_size_rewrite_invalidates_reference_cache(tmp_path):
    ...
    vae, encoder = _encoder()
    first = encoder._encode(str(path), "right")
    changed = [1000] * 20000
    changed[10000] = -1000
    ...
    second = encoder._encode(str(path), "right")
    assert len(vae.inputs) == 2
    assert not torch.equal(first, second)
    assert encoder._service.stats()["misses"] == 2
...
def test_equal_sized_references_do_not_collide(tmp_path):
    ...
    vae, encoder = _encoder()
    first = encoder._encode(str(a), "right")
    second = encoder._encode(str(b), "right")
    assert not torch.equal(first, second)
    assert len(vae.inputs) == 2
...
def test_padding_sides_use_separate_reference_cache_entries(tmp_path):
    ...
    vae, encoder = _encoder()
    right = encoder._encode(str(path), "right")
    left = encoder._encode(str(path), "left")
    ...
    torch.testing.assert_close(encoder._encode(str(path), "right"), right)
    torch.testing.assert_close(encoder._encode(str(path), "left"), left)
    assert len(vae.inputs) == 2
